...
if (!RESTRICT_ALL || User::Logged())
{
  ...
  {
    if ( Idea::Remove(Idea::GetById($id),User::$current) )
    {
  ...
  if ( isset($_POST["rate_plus"]) || isset($_POST["rate_minus"]) )
  {
    ...
    if (empty($error_message))
    {
      ...
      {
        if (isset($_POST["rate_plus"]))  Idea::RatePlus($idea,User::$current);
        if (isset($_POST["rate_minus"])) Idea::RateMinus($idea,User::$current);
